local hkrul_sceolan_defeat = {
	["hkrul_sceolan"] = {trait = "hkrul_defeated_trait_sceolan"}
}


local function hkrul_sceolan_get_enemy_legendary_lords_in_last_battle(character)
	...
	for i = 1, num_attackers do
		...
		if char_obj:is_null_interface() == false then
			...
			if char_subtype == "hkrul_sceolan" then
...
core:add_listener(
	"hkrul_sceolan_defeat",
	"CharacterCompletedBattle",
	true,
	function(context)
		local character = context:character()
		if cm:char_is_general_with_army(character) and character:won_battle() then
			local enemy_LL = hkrul_sceolan_get_enemy_legendary_lords_in_last_battle(character)
			
			for i = 1, #enemy_LL do
				local LL_details = hkrul_sceolan_defeat[enemy_LL[i]]
