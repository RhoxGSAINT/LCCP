local hkrul_zach_defeat = {
	["hkrul_zach"] = {trait = "hkrul_defeated_trait_zach"}
}


local function hkrul_zach_get_enemy_legendary_lords_in_last_battle(character)
	...
	for i = 1, num_attackers do
		...
		if char_obj:is_null_interface() == false then
			...
			if char_subtype == "hkrul_zach" then
...
core:add_listener(
	...
	function(context)
		...
		if cm:char_is_general_with_army(character) and character:won_battle() then
			local enemy_LL = hkrul_zach_get_enemy_legendary_lords_in_last_battle(character)
			
			for i = 1, #enemy_LL do
				local LL_details = hkrul_zach_defeat[enemy_LL[i]]
